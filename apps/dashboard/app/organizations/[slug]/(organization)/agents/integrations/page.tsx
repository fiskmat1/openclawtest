...
import {
  agentControlChannelStatusLabels,
  agentControlChannelTypeLabels,
...
import { getAgentControlChannels } from '~/data/agents/get-agent-control-channels';
...
export default async function AgentIntegrationsPage(): Promise<React.JSX.Element> {
  const [connections, browserProfiles, controlChannels, teams] =
    await Promise.all([
      getProviderConnections(),
      getBrowserProfiles(),
      getAgentControlChannels(),
      getAgentTeams()
    ]);
  ...
  return (
    ...
    >
      <div className="grid gap-6 xl:grid-cols-3">
                    ...
                    <TableRow key={connection.id}>
                      ...
                      <TableCell>
                        <AgentStatusBadge
                          ...
                          label={
                            providerConnectionStatusLabels[connection.status]
                          }
                        />
                      </TableCell>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Control channels</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {controlChannels.length > 0 ? (
                    ...
                    <TableRow key={channel.id}>
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {agentControlChannelTypeLabels[channel.type]} via{' '}
                          {channel.providerConnectionName}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <AgentStatusBadge
                          ...
                          label={
                            agentControlChannelStatusLabels[channel.status]
                          }
                        />
                      </TableCell>
                      <TableCell className="text-sm text-muted-foreground">
                        ...
                        {channel.externalThreadId
                          ? ` / thread ${channel.externalThreadId}`
                          : ''}
                      </TableCell>
                    </TableRow>
                  ))}
              ...
              <div className="p-6 text-sm text-muted-foreground">
                No control channels yet. Add a Telegram integration, then send{' '}
                <code>/bind team-slug</code> from the target chat to attach it
                to a team.
              </div>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="p-0">
            {browserProfiles.length > 0 ? (
              ...
              <div className="p-6 text-sm text-muted-foreground">
                No browser profiles yet. These are legacy Kernel/OpenClaw
                profiles; the primary live view now comes from E2B runtime
                control URLs.
              </div>
            )}
          </CardContent>
